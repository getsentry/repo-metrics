...
pub fn render_term(o: &Output) -> String {
    ...
    match o {
        ...
        Output::Tree { root, .. } => {
            fn walk(n: &TreeNode, depth: usize, total: u64, out: &mut String, st: &Style) {
                let pct = if total > 0 {
                    (n.size as f64 / total as f64) * 100.0
                } else {
                    0.0
                ...
                out.push_str(&format!(
                    ...
                    "  ".repeat(depth),
                    n.name,
                    st.dim(&format!("{:>9}", human_bytes(n.size))),
                    st.dim(&format!("{pct:>5.1}%  {} files", group(n.files as i64)))
                ));
                for c in &n.children {
                    walk(c, depth + 1, total, out, st);
                }
            }
            walk(root, 0, root.size, &mut out, &st);
        }
    }
    out.push('\n');
    out
